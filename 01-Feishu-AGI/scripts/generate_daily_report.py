...
def export_doc_to_markdown(doc_token, access_token):
    ...
    r = requests.get(url, headers={'Authorization': f'Bearer {access_token}'}, params={'lang': 0})
    data = r.json()
    if data.get('code') != 0:
...
def load_index_map():
    index_file = Path(__file__).parent.parent / 'data' / 'index_map.json'
...
def generate_daily_report(date, access_token, output_dir):
    ...
    for i, token in enumerate(tokens, 1):
        ...
        content = export_doc_to_markdown(token, access_token)
        emoji = emojis[(i - 1) % len(emojis)]
        merged += f'## {emoji} 文章 {i}\n\n> 文档 ID: `{token}`\n\n'
        if content:
            print(f'[INFO] 正在提纯第 {i} 篇...', file=sys.stderr)
            refined = refine_content_with_claude(content, token)
